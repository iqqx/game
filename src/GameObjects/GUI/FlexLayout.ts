...
export class FlexLayout extends Layout {
	private readonly _justify: "START" | "CENTER" | "END" | "SPACE";
	private readonly _align: "START" | "CENTER" | "END";
	private readonly _gap: number;
	private readonly _horizontal: boolean;

	constructor(
		justify: "START" | "CENTER" | "END" | "SPACE" = "START",
	...
	public Repack() {
		...
		for (let i = 0; i < this._childs.length; ++i) {
			if (this._horizontal) totalHeight = Math.max(this._childs[i].Height, totalHeight);
			else totalHeight += this._childs[i].Height;
		}
