...
export class Blood extends GameObject {
    ...
    Update(dt) {
        if (this._freezed) {
            if (this._timeFromFreeze > 5000 && !this._rotated)
                return;
    ...
    Render() {
        ...
        if (this._freezed) {
            if (this._rotated) {
                ...
                Canvas.DrawEllipse(this._x - Scene.Current.GetLevelPosition(), this._y, 2 + 15 * Math.min(this._timeFromFreeze / 1000, 1), 2);
        }
        else
