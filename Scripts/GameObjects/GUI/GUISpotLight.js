...
export class GUISpotLight extends GUIBase {
    ...
    _palette = [Color.White, Color.Red, Color.Blue];
    _nextColor = 0;
    constructor() {
        super();
        for (let i = 0; i < 200; ++i) {
    ...
    Update(dt) {
        ...
        for (const dot of this._dots) {
            dot.lifeTime += dt;
            if (dot.lifeTime > this._lifeTime)
    ...
    Render() {
        ...
        GUI.ClearFillColor();
        for (const dot of this._dots)
            GUI.DrawCircleWithGradient(dot.Position.X, dot.Position.Y, 500 * (dot.lifeTime / (this._lifeTime * 0.5)), new Color(dot.Color.R, dot.Color.G, dot.Color.B, dot.lifeTime < this._lifeTime * 0.5 ? 25 * (dot.lifeTime / (this._lifeTime * 0.5)) : 25 * (1 - (dot.lifeTime - this._lifeTime * 0.5) / (this._lifeTime * 0.5))), Color.Transparent);
    }
}
