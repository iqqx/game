...
export class Monster extends Enemy {
    ...
    _sprites = {
        Idle: GetSprite("Monster_Idle"),
        Dead: GetSprite("Monster_Dead"),
        Jump: GetSprite("Monster_Jump"),
    };
    ...
    Render() {
        ...
        if (!this._grounded) {
            if (this.Direction === Direction.Right)
                Canvas.DrawImage(this._sprites.Jump, new Rectangle(this._x, this._y, this.Width, this.Height));
            else
                Canvas.DrawImageFlipped(this._sprites.Jump, new Rectangle(this._x, this._y, this.Width, this.Height));
            return;
        }
        const ddiv = Scene.Time % 3000;
        let frame;
        if (ddiv < 800)
            frame = this._sprites.Idle[0]; // Стоит
        else if (ddiv < 1200)
            frame = this._sprites.Idle[1]; // моргнул Стоит
        else if (ddiv < 2000)
            frame = this._sprites.Idle[0]; // Стоит
        else
            frame = this._sprites.Idle[2]; // Сел
        if (this.Direction === Direction.Right)
